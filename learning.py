...
class Agent:
    ...
    def __init__(self) -> None:
        ...
        self.model = Linear_QNet(625, 1024, 1)
    ...
    def get_action(self, state: numpy.ndarray):
        ...
        if random.randint(0, 200) < self.epsilon:
            final_move = random.random()*2*np.pi
        else:
            state0 = torch.tensor(state, dtype=torch.float)
            prediction = self.model(state0)
            final_move = prediction.item()
        return final_move
...
class TerranNoobgam(BotAI):
    ...
    async def on_step(self, iteration):
        ...
        if self.state_new.done_reward is None:
            self.action = self.agent.get_action(self.state_old.state)  # angle
            unit = self.units(UnitTypeId.HELLION).first
            x = np.cos(self.action)
            y = np.sin(self.action)
            unit.move(Point2((unit.position.x + x, unit.position.y + y)))
            logging.info(f'moved to angle {self.action}')
        else:
